...
#include "../../../modules/mod-spell-regulator-master/src/SpellRegulator.h"
...
#endif
                    if (damage > 0 && !HasSpellCooldown(triggeredByAura->GetSpellInfo()->Id))
                    {
            ...
            if (player)
            {
                ...
                if (!nearMembers.empty())
                {
                    ...
                    if (Unit* target = nearMembers.front())
                    {
                        ...
                        target->AddSpellCooldown(spellProto->Id, 0, 1.5*IN_MILLISECONDS);

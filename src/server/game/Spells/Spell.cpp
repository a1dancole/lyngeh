...
void Spell::cast(bool skipCheck)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->Id == 8690)
        m_caster->ToPlayer()->AddSpellCooldown(8690, 6948, 10 * MINUTE * IN_MILLISECONDS, true, false);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!hasNonDispelEffect && !hasDispellableAura && dispelMask && !IsTriggered())
    {
        if (Unit* target = m_targets.GetUnitTarget())
        {
            ...
            //Allow certain non-standard dispels only to work inside raids for healers only
            if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->IsFriendlyTo(target)) &&
                (m_caster->getClass() == CLASS_PRIEST && (dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_POISON)) || dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_CURSE)))) ||
                (m_caster->getClass() == CLASS_PALADIN && dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_CURSE))) ||
                (m_caster->getClass() == CLASS_DRUID && (dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_DISEASE)) || dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_MAGIC)))) ||
                m_caster->getClass() == CLASS_SHAMAN && (dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_MAGIC)) || dispelMask == SpellInfo::GetDispelMask(DispelType(DISPEL_CURSE))))
                if ((!m_caster->GetMap()->IsRaid() || !m_caster->ToPlayer()->HasHealSpec()) && m_caster->IsFriendlyTo(target))
                    return SPELL_FAILED_NOTHING_TO_DISPEL;

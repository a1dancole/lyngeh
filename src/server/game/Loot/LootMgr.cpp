...
// Selects invalid loot items to be removed from group possible entries (before rolling)
struct LootGroupInvalidWithClassSelector : public acore::unary_function<LootStoreItem*, bool>
{
    explicit LootGroupInvalidWithClassSelector(Loot const& loot, uint16 lootMode, const Player* player) : _loot(loot), _lootMode(lootMode), _player(player) { }
    ...
    bool operator()(LootStoreItem* item) const
    {
        ...
        if (!item->HasClassInGroup(_player))
            return true;
...
//Checks if player is in a group, and that the rolled item can be useable by a class in the group
bool LootStoreItem::HasClassInGroup(Player const* player) const
{
    if (Group const* group = player->GetGroup()) //Only need to bother if we're in a group
    {
        ItemTemplate const* itemInfo = sObjectMgr->GetItemTemplate(this->itemid);
        bool isUseable = false;

        if (itemInfo && itemInfo->AllowableClass) //Does the item have a class requirement?
        {
            Group::MemberSlotList const& members = group->GetMemberSlots();

            for (Group::member_citerator itr = members.begin(); itr != members.end(); ++itr)
            {
                if (Player* groupMember = ObjectAccessor::GetPlayer(*player, itr->guid))
                {
                    if(groupMember->IsInWorld()) // Logged on
                        if(groupMember->GetMapId() == player->GetMapId()) //Same Map
                            if (itemInfo->AllowableClass & groupMember->getClassMask())
                                isUseable = true;
                }
            }
        }
    ...
    {
        //Just deal with the player
        ItemTemplate const* itemInfo = sObjectMgr->GetItemTemplate(this->itemid);
        if (itemInfo->AllowableClass && itemInfo->AllowableClass & player->getClassMask())
            return true;

        return false;
    }

    return true; //We got here so just let the loot be added
}
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, Player const *player, LootStore const& store, uint16 lootMode) const
{
    ...
    possibleLoot.remove_if(LootGroupInvalidWithClassSelector(loot, lootMode, player));

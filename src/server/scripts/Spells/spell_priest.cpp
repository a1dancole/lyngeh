...
enum PriestSpells
{
    ...
    SPELL_BORROWED_TIME_TRIGGERED                   = 59891,
...
//-47757 & -47758 penance triggered spell
class spell_pri_penance_trigger : public SpellScriptLoader
{
public:
    spell_pri_penance_trigger() : SpellScriptLoader("spell_pri_penance_trigger") { }

    class spell_pri_penance_trigger_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_pri_penance_trigger_SpellScript);
        ...
        {
            if (Unit* caster = GetCaster())
                if (Aura* borrowedTime = caster->GetAura(SPELL_BORROWED_TIME_TRIGGERED))
                    caster->RemoveAura(borrowedTime);
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_pri_penance_trigger_SpellScript::HandleAfterCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_pri_penance_trigger_SpellScript;
    }
};
        ...
        class spell_pri_shadow_word_death_SpellScript : public SpellScript
        {
            ...
            void HandleDamage()
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    caster->CastCustomSpell(GetCaster(), SPELL_PRIEST_SHADOW_WORD_DEATH, &damage, 0, 0, true);
                }
            }

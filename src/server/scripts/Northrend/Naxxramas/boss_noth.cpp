...
enum Events
{
    ...
    EVENT_SUMMON_FURIOUS_SKELETON           = 10
};
...
public:
    ...
    {
        explicit boss_nothAI(Creature *c) : BossAI(c, BOSS_NOTH), summons(me), justBlinked(false)
        {
        ...
        void StartGroundPhase()
        {
            ...
            if (Is25ManRaid())
            {
                ...
                events.ScheduleEvent(EVENT_SUMMON_FURIOUS_SKELETON, 10000);
            }
        }
        ...
        void Reset() override
        {
            ...
            if (InstanceScript* pInstance = me->GetInstanceScript())
                pInstance->DoRemoveAurasDueToSpellOnPlayers(42716);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MOVE_TO_GROUND:
                    ...
                    me->NearTeleportTo(nothPosition.GetPositionX(), nothPosition.GetPositionY(), nothPosition.GetPositionZ(), nothPosition.GetOrientation(), true);
                    break;
                case EVENT_SUMMON_FURIOUS_SKELETON:
                    me->MonsterTextEmote("A Furious Skeleton enters the fight", 0, true);
                    SummonHelper(NPC_FURIOUS_SKELETON, 1);
                    events.RepeatEvent(30000);
                    break;
            }

            if (events.IsInPhase(0))
            {
...
class npc_furious_skeleton : public CreatureScript
{
public:
    npc_furious_skeleton() : CreatureScript("npc_furious_skeleton") { }

    struct npc_furious_skeletonAI : public ScriptedAI
    {
        explicit npc_furious_skeletonAI(Creature* c) : ScriptedAI(c) { }
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            {
                target->CastSpell(target, 42716, true);
                me->CastSpell(target, 72301, true);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Unit* target = me->GetVictim())
                if (me->GetDistance(target->GetPosition()) <= 2.0f)
                    me->DealDamage(me, target, target->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
        }
    };

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_furious_skeletonAI(creature);
    }
};

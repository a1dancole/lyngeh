    ...
    struct boss_gluthAI : public BossAI
    {
        ...
        void JustSummoned(Creature *summon) override
        {
            if (summon->GetEntry() == NPC_ZOMBIE_CHOW)
            {
                ...
                summon->SetMaxHealth(RAID_MODE(21600, 33800));
            }
        ...
        void KilledUnit(Unit* who) override
        {
            if (me->IsAlive() && who->GetEntry() == NPC_ZOMBIE_CHOW)
                me->ModifyHealth(int32(me->GetMaxHealth()*RAID_MODE(0.05, 0.1)));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SPELL_MORTAL_WOUND:
                    ...
                    {
                        if (Aura* aur = victim->GetAura(SPELL_MORTAL_WOUND))
                        {
                            //Let 5 stacks drop off
                            if (aur->GetStackAmount() == RAID_MODE(5,6))
                            {

...
enum GameObjects
{
    GO_DEAD_PORTAL = 3500001,
    GO_LIVE_PORTAL = 3500002
};
...
struct OnSameSide : public acore::unary_function<Unit*, bool>
{
    bool m_inLiveSide;
    explicit OnSameSide(Unit* pSource) : m_inLiveSide(IN_LIVE_SIDE(pSource)) {}
...
public:
    ...
    struct boss_gothikAI : public BossAI
    {
        ...
        void EnterCombat(Unit * who) override
        {
            ...
            //events.ScheduleEvent(EVENT_CHECK_PLAYERS, 120000); This would be the mechanic where you can overload one side and the gate will open.
        ...
        void SummonHelpers(uint32 entry)
        {
            ...
            switch(entry)
            {
                case NPC_LIVING_TRAINEE:
                    me->SummonCreature(NPC_LIVING_TRAINEE, PosSummonLiving[pos].GetPositionX(), PosSummonLiving[pos].GetPositionY(), PosSummonLiving[pos].GetPositionZ(), PosSummonLiving[pos].GetOrientation());
                ...
                case NPC_LIVING_KNIGHT:
                    me->SummonCreature(NPC_LIVING_KNIGHT, PosSummonLiving[pos].GetPositionX(), PosSummonLiving[pos].GetPositionY(), PosSummonLiving[pos].GetPositionZ(), PosSummonLiving[pos].GetOrientation());
                    if (Is25ManRaid())
                ...
                case NPC_LIVING_RIDER:
                    me->SummonCreature(NPC_LIVING_RIDER, PosSummonLiving[pos].GetPositionX(), PosSummonLiving[pos].GetPositionY(), PosSummonLiving[pos].GetPositionZ(), PosSummonLiving[pos].GetOrientation());
                    break;
            }
        }
        ...
        void SpellHit(Unit * /*caster*/, const SpellInfo* spellInfo) override
        {
            ...
            switch (spellInfo->Id)
            {
                ...
                case SPELL_INFORM_LIVING_KNIGHT:
                    me->SummonCreature(RAID_MODE(NPC_DEAD_TRAINEE, NPC_DEAD_KNIGHT), PosSummonDead[pos].GetPositionX(), PosSummonDead[pos].GetPositionY(), PosSummonDead[pos].GetPositionZ(), PosSummonDead[pos].GetOrientation());
                    break;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_INTRO_4:
                    Talk(SAY_INTRO_4);
                    me->SummonCreature(GO_DEAD_PORTAL, 2693.216309, -3365.502686, 267.683228, 1, TEMPSUMMON_MANUAL_DESPAWN);
                    me->SummonCreature(GO_LIVE_PORTAL, 2692.514893, -3353.868896, 267.683228, 4, TEMPSUMMON_MANUAL_DESPAWN);
                    events.PopEvent();
                    break;
                ...
                case EVENT_SUMMON_ADDS:
                    ...
                    else
                    {
                        ...
                        me->AI()->AttackStartCaster(me->GetVictim(), 40.0f);
...
public:
    ...
    struct npc_boss_gothik_minionAI : public CombatAI
    {
        ...
        void EnterCombat(Unit*  /*who*/) override
        {
            ...
            switch (me->GetEntry())
            {
                ...
                case NPC_DEAD_TRAINEE:
                    events.ScheduleEvent(EVENT_SPELL_ARCANE_EXPLOSION, 1000);
                    break;
                case NPC_LIVING_KNIGHT:
                    events.ScheduleEvent(EVENT_SPELL_SHADOW_MARK, 2000);
                    break;
                case NPC_DEAD_KNIGHT:
                    events.ScheduleEvent(EVENT_SPELL_WHIRLWIND, 1000);
                    break;
                case NPC_LIVING_RIDER:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SPELL_ARCANE_EXPLOSION:
                    ...
                    {
                        if (IsDeadSide(victim) || gateOpened)
                        {
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                            me->SetControlled(false, UNIT_STATE_ROOT);
                            me->SetReactState(REACT_AGGRESSIVE);
                            me->AI()->AttackStartCaster(victim, 40.0f);
                            me->CastSpell(victim, 52660, false);
                        }
                        else
                        {
                            me->SetReactState(REACT_PASSIVE);
                ...
                case EVENT_SPELL_WHIRLWIND:
                    if (Unit* victim = me->GetVictim())
                    {
                        if (IsDeadSide(victim) || gateOpened)
                        {
                            ...
                            me->SetReactState(REACT_AGGRESSIVE);
                            me->AI()->AttackStartCaster(victim, 40.0f);
                            me->CastSpell(victim, 69972, false);
                        }
                        else
                        {
                            me->SetReactState(REACT_PASSIVE);
        ...
        private:
            bool IsDeadSide(Unit* victim)
            {
                return victim->GetPositionX() <= POS_X_NORTH &&
                    victim->GetPositionX() >= POS_X_SOUTH &&
                    victim->GetPositionY() >= POS_Y_GATE &&
                    victim->GetPositionY() <= POS_Y_WEST;
            }

            bool IsLiveSide(Unit* victim)
            {
                return victim->GetPositionX() <= POS_X_NORTH &&
                    victim->GetPositionX() >= POS_X_SOUTH &&
                    victim->GetPositionY() <= POS_Y_GATE &&
                    victim->GetPositionY() >= POS_Y_EAST;
            }
    };
...
class npc_gothik_dead_portal : public CreatureScript
{
public:
    npc_gothik_dead_portal() : CreatureScript("npc_gothik_dead_portal") { }

    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetInstanceAI<npc_gothik_dead_portalAI>(creature);
    }

    struct npc_gothik_dead_portalAI : public NullCreatureAI
    {
        npc_gothik_dead_portalAI(Creature* creature) : NullCreatureAI(creature)
        {
        }

        void OnSpellClick(Unit* player, bool& result)
        {
            if (Creature* livePortal = me->FindNearestCreature(GO_LIVE_PORTAL, 10.0f))
                if (player->GetTypeId() == TYPEID_PLAYER && player->ToPlayer()->HasHealSpec())
                    player->NearTeleportTo(livePortal->GetPositionX(), livePortal->GetPositionY(), livePortal->GetPositionZ(), player->GetPosition().GetOrientation(), false, false, true, true);
        }
    };
};

class npc_gothik_live_portal : public CreatureScript
{
public:
    npc_gothik_live_portal() : CreatureScript("npc_gothik_live_portal") { }

    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetInstanceAI<npc_gothik_live_portalAI>(creature);
    }

    struct npc_gothik_live_portalAI : public NullCreatureAI
    {
        npc_gothik_live_portalAI(Creature* creature) : NullCreatureAI(creature)
        {
        }

        void OnSpellClick(Unit* player, bool& result)
        {
            if (Creature* livePortal = me->FindNearestCreature(GO_DEAD_PORTAL, 10.0f))
                if (player->GetTypeId() == TYPEID_PLAYER && player->ToPlayer()->HasHealSpec())
                    player->NearTeleportTo(livePortal->GetPositionX(), livePortal->GetPositionY(), livePortal->GetPositionZ(), player->GetPosition().GetOrientation(), false, false, true, true);
        }
    };
...
void AddSC_boss_gothik()
{
    ...
    new npc_gothik_dead_portal();
    new npc_gothik_live_portal();
}

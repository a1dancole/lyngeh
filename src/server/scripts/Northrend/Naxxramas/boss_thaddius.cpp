...
public:
    ...
    struct boss_thaddiusAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (Creature* cr = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_STALAGG_BOSS)))
            {
                cr->SetReactState(REACT_AGGRESSIVE);
                cr->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                cr->SetControlled(false, UNIT_STATE_ROOT);
            }
            if (Creature* cr = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_FEUGEN_BOSS)))
            {
                cr->SetReactState(REACT_AGGRESSIVE);
                cr->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                cr->SetControlled(false, UNIT_STATE_ROOT);
        ...
        void DamageTaken(Unit* player, uint32& damage, DamageEffectType, SpellSchoolMask) override
        {
            if ((player->HasAura(SPELL_POSITIVE_POLARITY) && !player->HasAura(SPELL_POSITIVE_CHARGE_STACK)) || (player->HasAura(SPELL_NEGATIVE_POLARITY) && !player->HasAura(SPELL_NEGATIVE_CHARGE_STACK)))
                damage = 0;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                summonTimer += diff;
                if (summonTimer >= RAID_MODE(12000, 8000))
                {
            ...
            case EVENT_THADDIUS_START_2:
                ...
                if(Is25ManRaid())
                    events.ScheduleEvent(EVENT_THADDIUS_ATTRACT, 45000);
            ...
            case EVENT_ACTIVATE_BALL_LIGHTNING:
                ...
                events.PopEvent();
                break;
            case EVENT_THADDIUS_ATTRACT:
            {
                std::list<HostileReference*> t_list = me->getThreatManager().getThreatList();
                for (std::list<HostileReference*>::const_iterator itr = t_list.begin(); itr != t_list.end(); ++itr)
                {
                    Unit* pTarget = ObjectAccessor::GetUnit(*me, (*itr)->getUnitGuid());
                    if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER && pTarget->IsAlive())
                        me->CastSpell(pTarget, SPELL_MAGNETIC_PULL, true);
                }
    ...
    struct boss_thaddius_summonAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* pWho) override
        {
            ...
            if (me->GetEntry() == NPC_STALAGG)
            {
                ...
                {
                    if (itr->GetSource()->IsWithinDist(me, 35) && itr->GetSource()->IsAlive())
                        attackers.push_back(itr->GetSource());
                }
        ...
        void boss_thaddius_summonAI::AttackingFeugen(Creature* feugen)
        {
            feugen->SetReactState(REACT_AGGRESSIVE);
            feugen->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
            feugen->SetControlled(false, UNIT_STATE_ROOT);

            if (Creature* stalagg = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_STALAGG_BOSS)))
            {
                stalagg->SetReactState(REACT_PASSIVE);
                stalagg->AttackStop();
                stalagg->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                stalagg->SetControlled(true, UNIT_STATE_ROOT);
            }
        }

        void boss_thaddius_summonAI::AttackingStalagg(Creature* stalagg)
        {
            stalagg->SetReactState(REACT_AGGRESSIVE);
            stalagg->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
            stalagg->SetControlled(false, UNIT_STATE_ROOT);

            if (Creature* feugen = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_FEUGEN_BOSS)))
            {
                feugen->SetReactState(REACT_PASSIVE);
                feugen->AttackStop();
                feugen->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                feugen->SetControlled(true, UNIT_STATE_ROOT);
            }
        }

        void boss_thaddius_summonAI::DoPull()
        {
            if (pInstance)
                switch (me->GetEntry())
                {
                case NPC_STALAGG:
                    ...
                    {
                        Map::PlayerList const& players = me->GetMap()->GetPlayers();
                        std::vector<Player*> feugenAttackers;
                        std::vector<Player*> stalaggAttackers;
                        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                        {
                            if (feugen->IsWithinDist(itr->GetSource(), 35) && itr->GetSource()->IsAlive())
                                feugenAttackers.push_back(itr->GetSource());

                            if (me->IsWithinDist(itr->GetSource(), 35) && itr->GetSource()->IsAlive())
                                stalaggAttackers.push_back(itr->GetSource());
                        }

                        if (!stalaggAttackers.empty() && !feugenAttackers.empty())
                        {
                            //Bring all players to the same platform
                            if (feugen->IsAlive())
                                for (std::vector<Player*>::const_iterator itr = stalaggAttackers.begin(); itr != stalaggAttackers.end(); ++itr)
                                    feugen->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);
                            else if (me->IsAlive())
                                for (std::vector<Player*>::const_iterator itr = feugenAttackers.begin(); itr != feugenAttackers.end(); ++itr)
                                    me->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);

                            DoAction(ACTION_MAGNETIC_PULL);
                            me->IsAlive() ? AttackingStalagg(me) : AttackingFeugen(feugen);
                        }
                        else
                        {
                            if (!stalaggAttackers.empty())
                            {
                                for (std::vector<Player*>::const_iterator itr = stalaggAttackers.begin(); itr != stalaggAttackers.end(); ++itr)
                                    feugen->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);

                                feugen->AI()->DoAction(ACTION_MAGNETIC_PULL);
                                AttackingFeugen(feugen);
                            }

                            if (!feugenAttackers.empty())
                            {
                                for (std::vector<Player*>::const_iterator itr = feugenAttackers.begin(); itr != feugenAttackers.end(); ++itr)
                                    me->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);
                ...
                case NPC_FEUGEN:
                    if (Creature* stalagg = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_STALAGG_BOSS)))
                    {
                        ...
                        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                        {
                            if (stalagg->IsWithinDist(itr->GetSource(), 35) && itr->GetSource()->IsAlive())
                                feugenAttackers.push_back(itr->GetSource());
                        ...
                        if (!stalaggAttackers.empty() && !feugenAttackers.empty())
                        {
                            ...
                            if (stalagg->IsAlive())
                                for (std::vector<Player*>::const_iterator itr = stalaggAttackers.begin(); itr != stalaggAttackers.end(); ++itr)
                                    stalagg->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);
                            else if (me->IsAlive())
                                for (std::vector<Player*>::const_iterator itr = feugenAttackers.begin(); itr != feugenAttackers.end(); ++itr)
                                    me->CastSpell(*itr, SPELL_MAGNETIC_PULL, true);

                            DoAction(ACTION_MAGNETIC_PULL);
                            me->IsAlive() ? AttackingFeugen(me) : AttackingStalagg(stalagg);
                        }
                        else
                        {
                            if (!stalaggAttackers.empty())
                            {
                                ...
                                stalagg->AI()->DoAction(ACTION_MAGNETIC_PULL);
                                AttackingStalagg(stalagg);
                            }

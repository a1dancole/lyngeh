...
enum Events
{
    ...
    EVENT_BERSERK = 5,
    EVENT_ENTER_COMBAT = 6,
    EVENT_ENTER_COMBAT_BARON = 7,
    EVENT_ENTER_COMBAT_ZELIEK = 8
};
...
public:
    ...
    struct boss_four_horsemenAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (horsemanId == HORSEMAN_RIVENDARE || horsemanId == HORSEMAN_ZELIEK)
            {
            ...
            if (Is25ManRaid())
            {
                events.RescheduleEvent(EVENT_ENTER_COMBAT_BARON, 40000);
                events.RescheduleEvent(EVENT_ENTER_COMBAT_ZELIEK, 70000);
            }
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if(horsemanId == HORSEMAN_BLAUMEUX)
                if (Creature* sirZeliek = me->FindNearestCreature(NPC_SIR_ZELIEK, 100.0f, true))
                    sirZeliek->AI()->DoAction(EVENT_ENTER_COMBAT);

            if (horsemanId == HORSEMAN_KORTHAZZ)
                if (Creature* baron = me->FindNearestCreature(NPC_BARON_RIVENDARE, 100.0f, true))
                    baron->AI()->DoAction(EVENT_ENTER_COMBAT);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ENTER_COMBAT_ZELIEK:
                if (horsemanId == HORSEMAN_BLAUMEUX)
                    if (Creature* sirZeliek = me->FindNearestCreature(NPC_SIR_ZELIEK, 100.0f, true))
                        sirZeliek->AI()->DoAction(EVENT_ENTER_COMBAT);

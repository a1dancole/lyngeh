            ...
            switch(pGo->GetEntry())
            {
                ...
                case GO_SAPPHIRON_GATE:
                    ...
                    if (GetBossState(BOSS_SAPPHIRON) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_SAPPHIRON_PORTAL:
                    _sapphironPortalGUID = pGo->GetGUID();
                    if (GetBossState(BOSS_THADDIUS) == DONE && GetBossState(BOSS_MAEXXNA) == DONE && GetBossState(BOSS_HORSEMAN) == DONE && GetBossState(BOSS_LOATHEB) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                ...
                switch (bossId)
                {
                    ...
                    case BOSS_LOATHEB:
                        ...
                        if (GameObject* go = instance->GetGameObject(_plagueEyePortalGUID))
                            go->SetGoState(GO_STATE_ACTIVE);
                        if (GetBossState(BOSS_THADDIUS) == DONE && GetBossState(BOSS_MAEXXNA) == DONE && GetBossState(BOSS_HORSEMAN) == DONE && GetBossState(BOSS_LOATHEB) == DONE)
                            if(GameObject * go = instance->GetGameObject(_sapphironPortalGUID))
                                go->SetGoState(GO_STATE_ACTIVE);
...
class sapphiron_area_trigger_teleport : public AreaTriggerScript
{
public:
    sapphiron_area_trigger_teleport() : AreaTriggerScript("sapphiron_area_trigger_teleport") { }

    bool OnTrigger(Player* player, const AreaTrigger* at)
    {
        if (InstanceScript* instance = player->GetInstanceScript())
        {
            if (instance->GetBossState(BOSS_THADDIUS) == DONE && instance->GetBossState(BOSS_MAEXXNA) == DONE && instance->GetBossState(BOSS_HORSEMAN) == DONE && instance->GetBossState(BOSS_LOATHEB) == DONE)
                return false;
...
void AddSC_instance_naxxramas()
{
    ...
    new sapphiron_area_trigger_teleport();
}

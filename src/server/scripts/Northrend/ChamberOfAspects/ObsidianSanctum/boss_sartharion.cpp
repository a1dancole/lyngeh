...
enum Events
{
    ...
    EVENT_LIFTOFF                               = 19,
    EVENT_LAND                                  = 20,
    EVENT_DEEP_BREATH                           = 21,
    EVENT_DEEP_BREATH_MOVEMENT                  = 22,
...
public:
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        void DoAction(int32 param)
        {
            ...
            {
                bool areDrakesAlive = false;
                me->CastSpell(me, SPELL_SARTHARION_TWILIGHT_REVENGE, true);

                for (uint8 i = 0; i < 3; ++i)
                {
                    if (dragons[i])
                        if (Creature* cr = ObjectAccessor::GetCreature(*me, dragons[i]))
                            if (cr->IsAlive())
                                areDrakesAlive = true;
                }

                if (!areDrakesAlive && me->IsFlying())
                {
                    me->SendMeleeAttackStop(me->GetVictim());
                    me->GetMotionMaster()->MoveLand(13, me->GetHomePosition().GetPositionX(), me->GetHomePosition().GetPositionY(), me->GetHomePosition().GetPositionZ(), 12.0f);
                    events.PopEvent();
                    DoResetThreat();
                }
            }
        ...
        void EnterCombat(Unit*  /*pWho*/)
        {
            ...
            events.ScheduleEvent(EVENT_SARTHARION_LAVA_STRIKE, 30000);
...
void boss_sartharion::boss_sartharionAI::HandleSartharionAbilities()
{
    ...
    switch (events.GetEvent())
    {
        ...
        {
            int32 bp0 = RAID_MODE(urand(500, 3000), urand(1000,4000));
            me->IsFlying() ? me->CastCustomSpell(SelectTarget(SELECT_TARGET_RANDOM), 68926, &bp0, NULL, NULL, false, 0, 0, me->GetGUID()) : me->CastSpell(me->GetVictim(), SPELL_SARTHARION_CLEAVE, false);
        ...
        case EVENT_LIFTOFF:
        {
            if (!me->IsFlying())
            {
                ...
                me->GetMotionMaster()->MoveTakeoff(1, 3243.851563, 493.373657, 75.0f, 12.0f);

                if (Is25ManRaid())
                {
                    nextPos = Position(3249.861328, 569.452759, 75.0f);
        ...
        case EVENT_DEEP_BREATH:
        {
            if (me->IsFlying())
            {
                me->MonsterTextEmote("Sartharion takes a deap breath!", 0, true);
                me->CastSpell(me, 18564, false);
                events.RepeatEvent(45000);
                events.DelayEvents(10000);
                events.ScheduleEvent(EVENT_DEEP_BREATH_MOVEMENT, 8250);
            }
            else
                events.PopEvent();
        ...
        case EVENT_DEEP_BREATH_MOVEMENT:
        {
            me->SetSpeed(MOVE_RUN, 3.5f, false);
            me->GetMotionMaster()->MovePoint(0, nextPos);
            events.PopEvent();
            nextPos = me->GetPositionX() == 3243.851563 ? Position(3249.861328, 569.452759, 75.0f) : Position(3243.851563, 493.373657, 75.0f);
        }
    }
}
...
void boss_sartharion::boss_sartharionAI::HandleDrakeAbilities()
{
    ...
    switch (events.GetEvent())
    {
        ...
        case EVENT_SARTHARION_CALL_TENEBRON:
            ...
            events.ScheduleEvent(EVENT_LIFTOFF, 13000);
            events.PopEvent();
            break;
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_HATCH_EGGS:
                {
                    ...
                    for (SummonList::iterator i = summons.begin(); i != summons.end(); ++i)
                    {
                        if ((cr = ObjectAccessor::GetCreature(*me, *i)))
                        {
                            if (cr->GetEntry() == NPC_TWILIGHT_EGG && cr->IsAlive())
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                    ...
                    if (Creature* cr = me->SummonCreature((isSartharion ? NPC_ACOLYTE_OF_VESPERON : NPC_DISCIPLE_OF_VESPERON), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 45000))
                    {
                        summons.Summon(cr);
                        cr->SetPhaseMask(16, true);
                    }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (checkAuraTimer >= 1000)
            {
                ...
                for (const auto& i : PlList)
                {
                    ...
                    {
                        if (player->IsGameMaster() || !player->InSamePhase(me))
                            continue;

                        if (player->IsAlive())
                            if (player->HasAura(SPELL_TWILIGHT_TORMENT_VESPERON))
                                player->RemoveAura(SPELL_TWILIGHT_TORMENT_VESPERON);
                    }
                }
                checkAuraTimer = 0;
            }

            if (shadowBoltVolleyTimer >= 3000)
            {
                me->CastSpell(me, 40721, false);
                shadowBoltVolleyTimer = 0;
            }
